...
if (isset($_POST['send'])) {
    ...
    $result = $voucherHandler->claimVoucher($_SESSION['voucher'], $passcode);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="main">
        ...
        <div class="title">
            <span style="font-weight: 700;">Redeme Voucher</span>
        </div>
        <div class="action">
            Silahkan berikan ponsel anda ke kasir yang sedang beroprasi untuk melakukan redeme voucher anda
            <form method="POST" style="margin-top: 10px;">                           
                <div class="input-group">
                    <span class="input-group-text"><i data-feather="file-text" class="feather-sm fill-white"></i></span>
                    <input type="password" class="form-control" name="password" placeholder="Pass Code" aria-label="Username" aria-describedby="basic-addon1">
                </div>
                <?php
                if(@$result['message']){
                ?>
                <div class="input-group badge w-100 bg-light-danger text-danger">
                    <span class="input-group-text"><?= @$result['message']; ?></span>
                </div>
                <?php    
                }
                ?>
                <button type="submit" name="send" class="btn"><i class="bi bi-arrow-clockwise"></i> Redeme</button>
            </form>
        </div>
    </div>
    <footer>
