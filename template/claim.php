...
if (isset($_POST['claim'])) {
    ...
    $_SESSION['idoutlet']=$_POST['idoutlet'];
    ...
    if ($result) {
        ...
        $Error = "Maaf nomor telepon yang anda masukkan telah digunakan!";
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="main">
        ...
        <div class="title">
            Baru! Ada voucher spesial buat khusus untuk Komunitas whatsapp Pizzahut Delivery (kuota terbatas)
        </div>
        <div class="action">
            <form method="post">
                <input placeholder="Masukkan nomor telepon" autofocus="autofocus" id="phone_number" name="phone_number">
                <?php if(@$Error){
                    echo "<div class='mb-2 badge w-100 bg-light-danger text-danger' style='text-align:left;'><i class='ti ti-alert-circle'></i> {$Error}</div>";
                } ?>
                <input type="hidden" name="idoutlet" value="<?= $_SESSION['idoutlet']; ?>">
                <button id="claim" name="claim" class="btn" disabled>
                Klaim Voucher
                </button>
                <div class="phone-hint">+62</div>
            </form>
        </div>
    </div>
    <script>
        $(document).ready(function() {
            $('#phone_number').on('input', function() {
                ...
                let claimButton = $('#claim');

                claimButton.prop('disabled', !isPhone);

                // Change background color based on the disabled state
                claimButton.css('background-color', claimButton.prop('disabled') ? 'gray' : '');
            });
        });
    </script>
</body>
