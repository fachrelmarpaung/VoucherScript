...
$outletData = $outletHandler->getOutletById($_GET['id']);
...
if (isset($_POST['submit'])) {
    ...
    $outletHandler = new OutletHandler();
    $result = $outletHandler->updateOutlet($myid,$idoutlet, $outletName, $linkKomunitas);

    if ($result) {
        // Outlet successfully updated
        // Redirect to a confirmation page or any other desired location
        header("Location: Outlet");
        exit;
    } else {
        // Handle the case where the update failed
        echo "Failed to update outlet. Please check the input data.";
    }
}
?>
<!DOCTYPE html>
              ...
              <div class="card">
                <div class="card-body">
                  <div class="d-md-flex align-items-center mb-9">
                    <div>
                      <h5 class="card-title fw-semibold">Tambah Outlet</h5>
                    </div>
                  </div>
                  <form method="post" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                    <div class="mb-3">
                        <input type="text" class="form-control" placeholder="Kode Outlet" value="<?= $outletData['idoutlet']; ?>" name="kodeoutlet">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" placeholder="Nama Outlet" value="<?= $outletData['outlatename']; ?>" name="namaoutlet">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" placeholder="Link Komunitas Outlet" value="<?= $outletData['linkkomunitas']; ?>" name="linkkominitas">
                    </div>
                    <div class="d-md-flex align-items-center">
                        <button type="submit" class="mt-3 mt-md-0 ms-auto btn btn-primary" name="submit"><i class="ti ti-building-store"></i> Edit Outlet</button>
                    </div>
                  </form>
                </div>
              </div>
