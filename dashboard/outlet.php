...
$outletHandler = new OutletHandler();
$LoadOutlet = $outletHandler->Listoutlet();
if(empty($_SESSION['user'])){
    header('Location: ../administrator');
}
if (empty(@$_GET['p'])) {
    echo "<script>window.location.replace('?p=1');</script>";
}elseif(@$_GET['search']){
    $LoadOutlet = $outletHandler->cariidOutlet($_GET['search']);
}if (isset($_POST['find'])) {
    if ($outletHandler->carikanOutlet($_POST['search']) > 0) {
...
if(isset($_POST['delete'])){
  $phonenum = $_POST['id'];
  $redirect = '?success';
  $result = $outletHandler->deleteOutlet($phonenum, $redirect); 
}
?>
<!DOCTYPE html>
<html lang="en">
              ...
              <div class="card">
                <div class="card-body">
                  <div class="d-md-flex align-items-center mb-9">
                    <div>
                      <h5 class="card-title fw-semibold">List Outlet</h5>
                      <p class="card-subtitle">Total Outlet <span class="badge bg-success"><?= $outletHandler->getAllData(); ?></span></p>
                    </div>
                    <div class="ms-auto mt-4 mt-md-0">
                      ...
                      <ul class="nav nav-tabs mt-2" role="tablist">
                        <li class="nav-item" role="presentation">
                          <a class="btn btn-primary" href="addoutlet">
                            <span><i class="ti ti-building-store"></i> Tambah Outlet</span>
                          </a>
                        </li>
                      </ul>
                    </div>
                      ...
                      <div class="table-responsive">
                        <table class="table align-middle mb-0 text-nowrap">
                          <tbody>
                            <tr>
                              <td class="ps-0">
                                <h6 class="mb-0 fw-semibold">Number</h6>
                              </td>
                              <td class="ps-0">
                                <div class="d-flex align-items-center gap-3">
                                  <h6 class="mb-0 fw-semibold">Kode Outlet</h6>
                                </div>
                              </td>
                              <td class="ps-0">
                                <h6 class="mb-0">Nama Outlet</h6>
                              </td>
                              <td class="ps-0">
                                <h6 class="mb-0">Link Komunitas</h6>
                              </td>
                            ...
                            <?php
                            if ($LoadOutlet !== false) {
                                if ($LoadOutlet->num_rows > 0) {
                                    $i = 1;
                                    $s = (($_GET['p'] * 10) - 10);
                                    while ($Number = $LoadOutlet->fetch_assoc()) {

                            ?>
                              <td class="ps-0">
                                <h6 class="mb-0 fw-semibold"><?php if ($_GET['p'] > 1) {
                                                                echo $s++ +1;
                                                            } else {
                                                                echo $i++;
                                                            } ?></h6>
                              ...
                              <td class="ps-0">
                                <div class="d-flex align-items-center gap-3">
                                  <div>
                                    <h6 class="mb-0 fw-semibold"><?= $Number['idoutlet'] ?></h6>
                                  </div>
                                </div>
                              </td>
                              <td class="ps-0">
                                <h6 class="mb-0"><?= $Number['outlatename'] ?></h6>
                              </td>
                              <td class="ps-0">
                                <a href="<?= $Number['linkkomunitas'] ?>" class="badge bg-light-primary text-primary rounded-pill"><i class="ti ti-brand-telegram"></i> Kunjungi</a>
                              </td>
                              <td class="text-end ps-0">
                                <form method="POST">
                                  <a href="../qrcode.php?link=<?= $_SERVER['SERVER_NAME']; ?>/?outlet=<?= $Number['idoutlet'] ?>" class="badge bg-light-primary text-primary rounded-pill"  style="margin-right:3px"><i class="ti ti-qrcode"></i></span></a>
                                  <a href="editOutlet?id=<?= $Number['id']; ?>" class="badge bg-light-success text-success rounded-pill"  style="margin-right:3px"><i class="ti ti-edit"></i></span></a>
                                  <input type="hidden" name="id" value="<?= $Number['id'] ?>">
                                  <button type="submit" name="delete" class="badge bg-light-danger text-danger rounded-pill" style="border:none;"><i class="ti ti-trash"></i></button>
                                </form>
                              </td>
                            </tr><?php
                                    }
                        ...
                        <nav aria-label="Page navigation" style="float:right" class="mt-2">
                            <ul class="pagination pagination-primary">
                                ...
                                if(empty($_GET['search'])){
                                    ...
                                    if ($_GET['p'] > 1) { ?>
                                        <li class="page-item"><a class="page-link" href="?p=<?= ($_GET['p'] - 1); ?>">Prev</a></li>
                                        <li class="page-item "><a class="page-link" href="?p=<?= ($_GET['p'] - 1); ?>"><?= ($_GET['p'] - 1); ?></a></li>
                                    <?PHP } elseif ($_GET['p'] > 2) { ?>
                                        <li class="page-item"><a class="page-link" href="?p=<?= ($_GET['p'] - 1); ?>">Prev</a></li>
                                        <li class="page-item "><a class="page-link" href="?p=<?= ($_GET['p'] - 2); ?>"><?= ($_GET['p'] - 2); ?></a></li>
                                        <li class="page-item "><a class="page-link" href="?p=<?= ($_GET['p'] - 1); ?>"><?= ($_GET['p'] - 1); ?></a></li>
                                    <?PHP } ?>
                                    <li class="page-item active"><a class="page-link" href="?p=<?= $_GET['p']; ?>"><?= $_GET['p']; ?></a></li>
                                    <?php if ($_GET['p'] < ($GLOBALS['jmlhhalaman'] - 1)) { ?>
                                        <li class="page-item"><a class="page-link" href="?p=<?= ($_GET['p'] + 1); ?>"><?= ($_GET['p'] + 1); ?></a></li>
                                        <li class="page-item"><a class="page-link" href="?p=<?= ($_GET['p'] + 2); ?>"><?= ($_GET['p'] + 2); ?></a></li>
                                        <li class="page-item"><a class="page-link" href="?p=<?= ($_GET['p'] + 1); ?>">Next</a></li>
                                ...
                                if ($GLOBALS['jmlhhalaman'] > 1) {
                                    if ($_GET['p'] > 1) { ?>
                                        <li class="page-item"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] - 1); ?>">Prev</a></li>
                                        <li class="page-item "><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] - 1); ?>"><?= ($_GET['p'] - 1); ?></a></li>
                                    <?PHP } elseif ($_GET['p'] > 2) { ?>
                                        <li class="page-item"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] - 1); ?>">Prev</a></li>
                                        <li class="page-item "><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] - 2); ?>"><?= ($_GET['p'] - 2); ?></a></li>
                                        <li class="page-item "><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] - 1); ?>"><?= ($_GET['p'] - 1); ?></a></li>
                                    <?PHP } ?>
                                    <li class="page-item active"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= $_GET['p']; ?>"><?= $_GET['p']; ?></a></li>
                                    <?php if ($_GET['p'] < ($GLOBALS['jmlhhalaman'] - 1)) { ?>
                                        <li class="page-item"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] + 1); ?>"><?= ($_GET['p'] + 1); ?></a></li>
                                        <li class="page-item"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] + 2); ?>"><?= ($_GET['p'] + 2); ?></a></li>
                                        <li class="page-item"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] + 1); ?>">Next</a></li>
                                    <?PHP } elseif ($_GET['p'] < $GLOBALS['jmlhhalaman']) { ?>
                                        <li class="page-item"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] + 1); ?>"><?= ($_GET['p'] + 1); ?></a></li>
                                        <li class="page-item"><a class="page-link" href="?search=<?= $_GET['search']?>&p=<?= ($_GET['p'] + 1); ?>">Next</a></li>
                                <?php } } } ?>
