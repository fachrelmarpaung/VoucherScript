...
if (isset($_POST['submit'])) {
    // Retrieve form data
    $idoutlet = $_POST["kodeoutlet"];
    $outletName = $_POST["namaoutlet"];
    $linkKomunitas = $_POST["linkkominitas"];
    
    $outletHandler = new OutletHandler();
    if ($outletHandler->inputOutlet($idoutlet, $outletName, $linkKomunitas)) {
        // Outlet data successfully inserted
        header("Location: Outlet");
        exit;
    } else {
        // Error occurred, handle the response
        $response = $outletHandler->response;

        if ($response['status'] === 'error') {
            $Error = $response['message'];

            // Handle specific errors if needed
            if (isset($response['errors']['idoutlet'])) {
                $Error .= '<br>' . $response['errors']['idoutlet'];
            }

            if (isset($response['errors']['insert'])) {
                $Error .= '<br>' . $response['errors']['insert'];
            }
        }
    }
}
              ...
              <div class="card">
                <div class="card-body">
                  <div class="d-md-flex align-items-center mb-9">
                    <div>
                      <h5 class="card-title fw-semibold">Tambah Outlet Baru</h5>
                    </div>
                  </div>
                  <form method="post" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                    <div class="mb-3">
                        <input type="text" class="form-control" placeholder="Kode Outlet"  name="kodeoutlet" required>
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" placeholder="Nama Outlet" name="namaoutlet" required>
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" placeholder="Link Komunitas Outlet" name="linkkominitas" required>
                    </div>
                    <div class="d-md-flex align-items-center">
                        <button type="submit" class="mt-3 mt-md-0 ms-auto btn btn-primary" name="submit"><i class="ti ti-building-store"></i> Tambah Outlet</button>
                    </div>
                  </form>
                </div>
              </div>
